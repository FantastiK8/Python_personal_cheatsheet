...
# Unfortunately, the list has a few shortcomings. The biggest issue is that it 
# can run into speed issues as it grows. The items in the list are stored next 
# to each other in memory, if the stack grows bigger than the block of memory 
# that currently holds it, then Python needs to do some memory allocations. 
...
# get() – Remove and return an item from the queue. If the queue is empty, wait 
...
# qsize() – Return the number of items in the queue. If no free slot is 
